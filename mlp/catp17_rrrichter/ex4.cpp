...
int main(){  
   ...
   for(i = 0; i < SIZE; i++){
      ...
      if (rand() % 2) vetor.push_back(new Pessoa("Ana Paula", Pessoa::FEMININO, time(0))); // adiciona no final do vetor
      else vetor.push_back(new Aluno("Jose Silva", Pessoa::FEMININO, time(0), "01001010", Aluno::GRADUACAO)); // adiciona no final do vetor
   }
