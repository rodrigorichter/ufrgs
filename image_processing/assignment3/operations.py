...
while rval:
	...
	if sobelIsActive:
		resultFrame = copy.deepcopy(cv2.Sobel(frame,cv2.CV_64F,1,0,ksize=5))
	if brightnessIsActive:
	...
	if horizontalFlipIsActive:
		resultFrame = cv2.flip(frame,1)
	if verticalFlipIsActive:
